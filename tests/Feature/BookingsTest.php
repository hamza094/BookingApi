...
class BookingsTest extends TestCase
{
    ...
    public function test_property_owner_does_not_have_access_to_bookings_feature()
    {
        ...
        private function create_apartment()
    {
        $owner = User::factory()->create()->assignRole(Role::ROLE_OWNER);
    ...
    public function test_user_can_get_only_their_bookings()
    {
        ...
        $response = $this->actingAs($user1)->withoutExceptionHandling()->getJson('/api/user/bookings');
        $response->assertStatus(200);
    ...
    $response->assertSee("not allowed to perform this action");
    }
    ...
    public function test_user_can_cancel_their_booking_but_still_view_it()
    {
        ...
        $response = $this->withoutExceptionHandling()->actingAs($user2)->deleteJson('/api/user/bookings/' . $booking->id);
            $response->assertSee("not allowed to perform this action");
